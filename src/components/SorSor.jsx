...
console.log(data);
        ...
        if (!cancelled) {
          ...
          Swal.fire({
            ...
            text: 'Anket oluşturmak için önce kategorilerin yüklenmesi gerekiyor.',
  ...
  const addOption = () => options.length < 6 && setOptions((prev) => [...prev, { text: '', image: null }]);
...
const [selectedOption, setSelectedOption] = useState(null);
  const handleSubmit = async (e) => {
    ...
    if (!question.trim() || !selectedOption.value) {
    ...
    try {
      ...
      if (!data.slug) {
        throw new Error('Sunucu oluşturulan ankete slug döndürmedi.');
      }

      // App state'ini tek kez güncelle.
      onPollCreated?.(data);

      await Swal.fire({
        icon: 'success',
        title: 'Anket yayında!',
        text: 'Topluluk artık senin için karar verebilir.',
        confirmButtonColor: '#0f172a',
        timer: 1200,
        showConfirmButton: false,
      });

      const targetUrl = `/anketler/${encodeURIComponent(data.slug)}`;
      console.log('YÖNLENDİRİLİYOR:', targetUrl);
      navigate(targetUrl);
    } catch (error) {
